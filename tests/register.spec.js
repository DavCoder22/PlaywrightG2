...
test.describe('Register Page - QA Audit', () => {
  ...
  test('DEBE validar contraseña mínima', async ({ page }) => {
    ...
    await page.click('button[type="submit"]');
    await expect(page.locator('[data-cy="auth-error"]')).toContainText(/al menos 6 caracteres/i);
  });
  ...
  test('DEBE validar contraseñas coinciden', async ({ page }) => {
    ...
    await page.click('button[type="submit"]');
    await expect(page.locator('[data-cy="auth-error"]')).toContainText(/no coinciden/i);
  });
